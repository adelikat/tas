...
local function _manipLv2()
    ...
    if _ragnarStr() > origStr then
        return _bail('Lv 2 Failed to get Str 0')
	end

    c.Log('Str Skip!')
    c.Save(5)
    c.Save('Lv1StrSkip-Delay-' .. delay)
	...
	if _ragnarLuck() > origLuck or _ragnarInt() > origInt then
		c.Log('Lv 2 Str + Agility + Vitality skip - delay ' .. delay)
		c.Save('Chp1Lv2-Str-Vit-Skip-ButOtherStats-Delay-' .. delay)
		return false
	end
