...
function _sampsonFirst()
	c.RandomFor(1)
	c.WaitFor(2)
	c.UntilNextInputFrame()
	c.WaitFor(2)
	c.RndAtLeastOne() -- Sampson appears
	c.RandomFor(19)
	c.UntilNextInputFrame()
    ...
    c.WaitFor(1) -- No delay was proven impossible
    c.DelayUpTo(2)
	...
	if c.ReadBattle() ~= 76 then
		return c.Bail('Sampson did not attack, not sure what he did')
	end
...
while not c.done do
	c.Load(100)
	local result = c.Cap(_sampsonFirst, 1000)
	if result then
		--c.Log('Turn manipulated')
		local cacheResult = c.AddToRngCache()
		if cacheResult then
			local result = c.Cap(_miss, 2)
			if result then
				c.Done()
			else
